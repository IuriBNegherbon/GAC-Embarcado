...
#define FIREBASE_TOKEN "eJsgHfmjEQo3yvHK9vUskFiiMdtcoSuXV1hWET9c"
//#define FIREBASE_HOST "console.firebase.google.com/project/gac-tcc"
#define WIFI_SSID "AndroidAP"
#define WIFI_PASSWORD "hqir9667"
#define minutos() ((millis()/60000)+1)
...
int tempoPisca = 350;
long previousMillis;
String hora, minuto, minutoGet, modoGAC = "ON";//Váriavel que armazenara o horario do NTP.
int tempoOff, minutosOff;
...
void setup() {
  ...
  //Serial.println(Firebase.getString("/teste"));
...
void Bd() {
  ...
  if (modoGAC != "OFF")
  {
    ...
    for (CarregaArray = 0; CarregaArray < 6; CarregaArray++) //Carrega as informações salvas na EEPROM para o awwat aHorario
    {
      ...
      //Serial.println(Firebase.getInt(var + "/user_data_rffffacao") > 0);
...
void Menu() //Cria tela do Menu
{
  ...
  display.drawString(10, 35, "Reiniciar");
  if (modoGAC == "OFF")
    display.drawString(10, 45, "Ajustar relogio");
  display.display();
  SMovimentaMe();
...
void SMovimentaMe() //movienta a seta do menu
{
  ...
  while (true) //Não sai da função até que seja informado
  {
    ...
    while (vbotaoDesce == LOW && vbotaoSobe == LOW && vbotaoOK == LOW)
    {
      ...
      if (vbotaoDesce == HIGH)
      {
        if ((linha < 35) || (modoGAC == "OFF" && linha < 45)) // DESCE
        {
      ...
      if (vbotaoOK == HIGH) // OK
      {
        ...
        {
          AjustaHorario("00","00","Relogio"); //Chama função para ajustar horario do GAC
        }
...
void AjustaHorario(String horarioStr, String minutoStr, String Tela)
{
  int coluna = 0, linha = 15, horario = 0, minuto = 0, f;
...
void VerificaHora()
{
  int vlrHora, minu = -1, dia = 1;
  hora = "";
  minuto = "";
  //Serial.println("Verificando hora");
  ...
  if (modoGAC != "OFF")
  {
    for (vlrHora = 0; vlrHora < 5; vlrHora++) //Pega os 5 primeiros caracteres da hora atual que é informada como hh:mm:ss
    {
      ...
      if (vlrHora == 4)
      {
        minuto = minuto + ntp.getFormattedTime().charAt(vlrHora); //Pega o minuto final
      }
    }

    if ((minuto == "4") || (minuto == "9"))
    {
      if (minutoGet != minuto)
      {
        Serial.println("Atualizando informaões com firebase");
        Bd();
        minutoGet = minuto;
      }
    }
  ...
  for (vlrHora = 0; vlrHora < 6; vlrHora++) //Compara a variavel "hora" com todos os registros salvos no array em busca de igualdade
  {
    ...
    //Serial.println(AjustaMinuto(aHorario[vlrHora][1]));
    //Serial.println(minutos());
    /*if (minu != minutos())
    {
      minu = minutos();
      Serial.println((aHorario[vlrHora][0] * 60) + aHorario[vlrHora][1] + minutosOff - tempoOff);
      Serial.println(minutos());
    }*/
    //Serial.println((aHorario[vlrHora][0] * 60) + aHorario[vlrHora][1] + minutosOff - tempoOff);
    if (minutos() >= 1440*dia)
      dia++;
    if ((modoGAC == "OFF") && ( (aHorario[vlrHora][0] * 60) + aHorario[vlrHora][1] + minutosOff - tempoOff == minutos() - ((dia - 1) * 1440)))
    {
      Serial.println("Alimentando cachorro");
      break;
    }
    else if (hora == AjustaHora(aHorario[vlrHora][0]) + ":" + AjustaMinuto(aHorario[vlrHora][1]) && aHorario[vlrHora][4] != 255)//Se a hora atual for igual à que definimos, irá acender o led.
    {
      Serial.println("Alimentando cachorro");
      break;
    }
